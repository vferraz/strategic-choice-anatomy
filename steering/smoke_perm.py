"""Deterministic gate for the permutation-null smoke (1 game, qwen). Exit 0 = PASS -> chain may start.

Checks (all hard), for the single mode h1_dinc:
  1. exactly one parquet, 168 rows (1 game x 4 cb x 2 layers x 3 variants (perm0/perm1/perm2) x 7 doses)
  2. variants == {perm0, perm1, perm2}; full cell structure: every (layer, variant, cb) has all 7 doses
  3. status == OK everywhere
  4. dose-0 rows: slot_pref_J identical across (layer, variant) within cb AND equal (4dp) to the
     substrate capture baseline pref_J/(pref_J+pref_P); realized_letter == substrate move_letter
     -> harness injects nothing at dose 0, bit-consistent with the capture run
  5. injection alive: max |slot_pref_J(dose!=0) - slot_pref_J(dose=0)| > 1e-3 somewhere
...
def main():
    ...
    for mode in ("h1_dinc",):
        ...
        check(set(df["variant"].unique()) == {"perm0", "perm1", "perm2"},
              f"{mode}: variants perm0/perm1/perm2 (found {sorted(df['variant'].unique())})")
        cell = df.groupby(["steer_layer", "variant", "counterbalance_id"])["dose"].nunique()
        check(len(cell) == 24 and (cell == 7).all(),
              f"{mode}: 24 cells x 7 doses (found {len(cell)} cells, doses {sorted(cell.unique())})")
